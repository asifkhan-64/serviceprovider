<?php
    ...
    $getEmail = $_GET['email'];
    $getCodeQuery = mysqli_query($connect, "SELECT * FROM customers WHERE email = '$getEmail'");
    ...
    if (isset($_POST['valideCode'])) {
        ...
        if ($userCode === $code) {
            $updateVerificationStatus = mysqli_query($connect, "UPDATE customers SET verification_status = '1', login_status = '1' WHERE email = '$getEmail'");
            if ($updateVerificationStatus) {
                header("LOCATION: thankYouCustomer.php");
            }
        }else {
            $tokenError = '
